...
export default function BlogGrid({ posts = [] }) {
  ...
  return (
    <section className="blog-page" style={{ padding: '120px 0', backgroundColor: 'var(--bg-light)' }}>
      <div className="container">
        <div className="section-title text-center sec-title-animation animation-style2" style={{ marginBottom: '60px' }}>
          <div className="section-title__tagline-box justify-content-center">
            <p className="section-title__tagline" style={{ color: 'var(--primary-color)' }}>Our Updates</p>
          </div>
        ...
        <div className="row">
          {posts.length > 0 ? (
            posts.map((item, index) => {
              ...
              return (
                <div className="col-xl-4 col-lg-4 col-md-6 wow fadeInUp" data-wow-delay={delay} key={item.id}>
                  <div className="blog-card" style={{ backgroundColor: 'white', borderRadius: '15px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', marginBottom: '30px', transition: 'all 0.3s ease' }}>
                    ...
                    <div className="blog-content" style={{ padding: '30px' }}>
                      ...
                      <h4 style={{ fontFamily: 'var(--font-prata)', marginBottom: '15px', fontSize: '1.3rem' }}>
                        <Link href={`/blog/${item.slug}`} style={{ color: 'var(--text-dark)' }}>{item.title}</Link>
                      </h4>
                      <p style={{ color: 'var(--text-grey)', marginBottom: '20px', fontSize: '0.95rem', lineHeight: '1.6' }}>
